...
<div class="wrap alm2 clear_fix specialPage">
  <div class="r">
      <div id="songs" class="list1">
          ...
          <form action="/home/special/edits/{{$gdid}}" method='post'>
            <ul>
                @foreach($rs as $k=>$v)  
                <li>
                  <a  hidefocus="true" href="javascript:;" >
                    <input type="checkbox"  class="cb checkItem" @if(strpos($res,"$v->mid")!== false) checked @endif id="chk_C3967DE92CF781436E29B2E30ACFA0E7" name='special_music[]' value="{{$v->mid}}">
            ...
            {{csrf_field()}}
            <input type="submit" id="button" class="kg_uc_btn_style02 pc_temp_b_btn" value="修改歌单" />
          </form>
      </div>
      <div class="clear"></div>
  </div>
