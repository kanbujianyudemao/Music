...
var orderConfirm = window.orderConfirm || {
		initEvent: function() {
			...
			Kg.$('#addAddress').addEvent('click',function(){
				...
				Kg.$('#address').val('');
				Kg.$('#editPop').show();
				// 解决编辑地址后，添加地址显示编辑地址问题
				$("#addressArea").find("option").attr("selected",false);
				if($("#addressArea").find("option").eq(0).attr("value") != 0){

					$("#addressArea").find("option").eq(0).before("<option value='0'>请选择</option>")
				}			
				$("#addressArea").val(0);

				$("#addressCity").find("option").attr("selected",false)
				if($("#addressCity").find("option").eq(0).attr("value") != 0){
					$("#addressCity").find("option").eq(0).before("<option value='0'>请选择</option>");
				}
				$("#addressCity").val(0);

				$("#addressTown").find("option").attr("selected",false)
				if($("#addressTown").find("option").eq(0).attr("value") != 0){
					$("#addressTown").find("option").eq(0).before("<option value='0'>请选择</option>");
				}
				$("#addressTown").val(0);
			...
			Kg.$('.address_edit').addEvent('click',function(){
				...
				getProvince(apply_address.getProvinceData())
...
function resetPlace(){
	...
	for(var i = 0, len = arguments.length; i < len; i++){
		...
		var that = $(arguments[i]),val = that.attr('data-v');
		
		// 解决firefox 只有默认项{请选择}和一个可选项{数据项}默认选不中 点击添加重新填写默认值
		if(navigator.userAgent.indexOf("Firefox") != -1 && navigator.userAgent.indexOf("Firefox") != undefined){	
			that.find('option').eq(0).remove();
		}
